...
class LookItDataset(data.Dataset):
    def __init__(self, opt):
        ...
        self.paths = self.collect_paths(
            "face_labels_fc"
        )  # change to "face_labels" if face classifier wasn't used
        # self.get_mean_std()
        self.img_processor = DataTransforms(
            self.opt.image_size, self.opt.per_channel_mean, self.opt.per_channel_std
        ).transformations[
            self.opt.phase
        ]  # ew.
    ...
    def get_mean_std(self, count=20000):
        ...
        for i in range(count):
            ...
            img = Image.open(
                self.opt.dataset_folder / "faces" / img_files_seg[2]
            ).convert("RGB")
    ...
    def collect_paths(self, face_label_name):
        ...
        for name in coding_names:
            stats[name] = {}
            gaze_labels = np.load(
                str(Path.joinpath(dataset_folder_path, name, f"gaze_labels.npy"))
            )
            gaze_labels_second = None
            if self.opt.use_mutually_agreed:
                second_label_path = Path.joinpath(
                    dataset_folder_path, name, f"gaze_labels_second.npy"
                )
                if second_label_path.exists():
                    gaze_labels_second = np.load(str(second_label_path))
            face_labels = np.load(
                str(Path.joinpath(dataset_folder_path, name, f"{face_label_name}.npy"))
            )
            ...
            stats[name][
                "other_fail_counter"
            ] = 0  # face detector failed or no annotation (must change preprocess.py to decouple them)
            logging.info("Video: {}".format(name))
            for frame_number in range(gaze_labels.shape[0]):
                gaze_label_seg = gaze_labels[
                    frame_number : frame_number + self.opt.sliding_window_size
                ]
                face_label_seg = face_labels[
                    frame_number : frame_number + self.opt.sliding_window_size
                ]
                ...
                if any(
                    face_label_seg < 0
                ):  # a tidy bit too strict?...we can basically afford 1 or two missing labels
                    ...
                    if cur_face >= 0:
                        ...
                        if cur_face == -1:
                            ...
                            raise ValueError
                    continue
                if not self.opt.eliminate_transitions or self.check_all_same(
                    gaze_label_seg
                ):
                    class_seg = gaze_label_seg[self.opt.sliding_window_size // 2]
                    if gaze_labels_second is not None:
                        gaze_label_second = gaze_labels_second[
                            frame_number + self.opt.sliding_window_size // 2
                        ]
                    ...
                    for i in range(self.opt.sliding_window_size):
                        img_files_seg.append(
                            f"{name}/img/{frame_number + i:05d}_{face_label_seg[i]:01d}.png"
                        )
                        box_files_seg.append(
                            f"{name}/box/{frame_number + i:05d}_{face_label_seg[i]:01d}.npy"
                        )
            ...
            logging.info(
                "extracted {} datapoints from {} frames.".format(
                    stats[name]["datapoints_counter"],
                    stats[name]["availble_datapoints"],
                )
            )
            logging.info(
                "window_fail: {}, coders_disagree: {}, fc_fail: {}, other_fail: {}\n".format(
                    stats[name]["window_not_fully_coded_counter"],
        ...
        total_ava_datapoints = np.sum(
            [
                stats[key]["availble_datapoints"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        total_used_datapoints = np.sum(
            [
                stats[key]["datapoints_counter"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        total_window_fail = np.sum(
            [
                stats[key]["window_not_fully_coded_counter"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        total_coders_disagree = np.sum(
            [
                stats[key]["coders_disagree_counter"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        total_fc_fail = np.sum(
            [
                stats[key]["fc_fail_counter"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        total_other_fail = np.sum(
            [
                stats[key]["other_fail_counter"]
                for key in stats.keys()
                if key != "video_counter"
            ]
        )
        ...
        logging.info(
            "dataset usage percent: {:.2f} %".format(
                100 * total_used_datapoints / total_ava_datapoints
            )
        )
        logging.info(
            "window fails: {:.2f} %".format(
                100 * total_window_fail / total_ava_datapoints
            )
        )
        logging.info(
            "coders disagree: {:.2f} %".format(
                100 * total_coders_disagree / total_ava_datapoints
            )
        )
        logging.info(
            "fc fails: {:.2f} %".format(100 * total_fc_fail / total_ava_datapoints)
        )
        logging.info(
            "other fails: {:.2f} %".format(
                100 * total_other_fail / total_ava_datapoints
            )
        )
        return my_list
    ...
    def __getitem__(self, index):
        ...
        if self.opt.horiz_flip:
            ...
            ):  # also do horizontal flip (but also swap label if necessary for left & right)
                flip = np.random.randint(2)
        imgs = []
        for img_file in img_files_seg:
            img = Image.open(self.opt.dataset_folder / "faces" / img_file).convert(
                "RGB"
            )
            if self.opt.rand_augment:
                if (
                    self.opt.phase == "train"
                ):  # compose random augmentations with post_processor
        ...
        for box_file in box_files_seg:
            box = np.load(
                self.opt.dataset_folder / "faces" / box_file, allow_pickle=True
            ).item()
            box = torch.tensor(
                [
                    box["face_size"],
...
class MyDataLoader:
    ...
    def plot_sample_collage(self, collage_size=25):
        ...
        assert np.sqrt(collage_size) == int(
            np.sqrt(collage_size)
        )  # collage size must have an integer square root
        random_dataloader = torch.utils.data.DataLoader(
            self.dataset, batch_size=self.opt.batch_size, shuffle=True, num_workers=0
        )  # use a random dataloader, with shuffling on so collage is of different children
        iterator = iter(random_dataloader)
        condition = (
            (len(bins[0]) < collage_size)
            or (len(bins[1]) < collage_size)
            or (len(bins[2]) < collage_size)
        )
        while condition:
            batch_data = next(iterator)
            for i in range(len(batch_data["label"])):
                if len(bins[batch_data["label"][i]]) < collage_size:
                    bins[batch_data["label"][i]].append(
                        batch_data["imgs"][i, 2, ...].permute(1, 2, 0)
                    )
        ...
        for class_id in classes.keys():
            ...
            imgs = (imgs - np.min(imgs, axis=(1, 2, 3), keepdims=True)) / (
                np.max(imgs, axis=(1, 2, 3), keepdims=True)
                - np.min(imgs, axis=(1, 2, 3), keepdims=True)
            )
            save_path = Path(
                self.opt.experiment_path,
                "{}_collage_{}.png".format(self.opt.phase, classes[class_id]),
            )
            visualize.make_gridview(
                imgs, ncols=int(np.sqrt(collage_size)), save_path=save_path
            )
