...
def parse_arguments_for_training():
    ...
    parser.add_argument(
        "dataset_folder", help="The path to the folder containing the data"
    )
    parser.add_argument(
        "--number_of_classes", type=int, default=3, help="number of classes to predict"
    )
    parser.add_argument(
        "--batch_size", type=int, default=16, help="Batch size to train with"
    )
    parser.add_argument(
        "--image_size",
        type=int,
    ...
    parser.add_argument(
        "--loss",
        type=str,
        choices=["cat_cross_entropy"],
        default="cat_cross_entropy",
    ...
    parser.add_argument(
        "--lr_policy",
        type=str,
        choices=["lambda", "plateau", "multi_step", "cyclic"],
        default="plateau",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0.98,
        help="Decay rate for lamda lr policy.",
    )
    parser.add_argument(
        "--continue_train",
        action="store_true",
        help="Continue training from latest iteration",
    )
    parser.add_argument(
        "--use_disjoint",
        action="store_true",
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed to train with"
    )
    parser.add_argument(
        "--gpu_id",
        type=str,
        default=-1,
        help="GPU ids to use, comma delimited (or -1 for cpu)",
    )
    parser.add_argument("--port", type=str, default="12355", help="port to use for ddp")
    parser.add_argument(
        "--tensorboard", action="store_true", help="Activates tensorboard logging"
    )
    parser.add_argument(
        "--log", action="store_true", help="Logs into a file instead of stdout"
    )
    parser.add_argument(
        "-v",
        "--verbosity",
...
def parse_arguments_for_testing():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=[None, "reference", "bbox"],
        help="Flag to use face recognition using either a reference image or bounding box annotation",
    )
    parser.add_argument(
        "--facerec_ref",
        type=str,
        default=None,
        help="Path to annotation or reference image",
    )
...
def parse_arguments_for_visualizations():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "output_folder", type=str, default="output", help="path to output results."
    )
    parser.add_argument(
        "human_codings_folder", type=str, help="the codings from human1"
    )
    parser.add_argument(
        "machine_codings_folder", type=str, help="the codings from machine"
    )
    parser.add_argument(
        "raw_dataset_type",
        type=str,
        choices=["lookit", "cali-bw", "senegal", "datavyu", "just_annotations"],
        default="lookit",
        help="the type of dataset to preprocess",
    )
    parser.add_argument(
        "--raw_dataset_path", type=str, help="path to raw dataset folder"
    )
    parser.add_argument(
        "--dataset_folder", type=str, help="path to preprocessed dataset folder"
    )
    parser.add_argument(
        "--human2_codings_folder", type=str, help="the codings from human2"
    )
    parser.add_argument(
        "--human_coding_format",
        type=str,
        default="lookit",
        choices=["lookit", "compressed", "vcx", "datavyu"],
    )
    parser.add_argument(
        "--machine_coding_format",
        type=str,
        default="compressed",
        choices=["lookit", "compressed", "vcx"],
    )
    parser.add_argument(
        "--unique_children_only",
        action="store_true",
        default=False,
    ...
    if args.raw_dataset_type == "lookit":
        ...
        if args.human_coding_format != "lookit":
            raise AssertionError(
                "For the just_annotations dataset type, only lookit coding format (for humans) is supported."
            )
    else:
    ...
    if args.human2_codings_folder:  # a second human annotation folder was passed
        ...
        if (
            not args.human_codings_folder.is_dir()
            or not args.human2_codings_folder.is_dir()
        ):
            if (
                args.raw_dataset_type == "lookit"
                or args.raw_dataset_type == "senegal"
                or args.raw_dataset_type == "cali-bw"
            ):
                import preprocess
                ...
                raise NotImplementedError(
                    "One of the human codings folder doesn't exist."
                )
...
def parse_arguments_for_preprocess():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "raw_dataset_path", type=str, help="path to raw dataset to preprocess"
    )
    parser.add_argument(
        "output_folder", type=str, help="path to put preprocessed dataset"
    )
    parser.add_argument(
        "--raw_dataset_type",
        type=str,
    ...
    parser.add_argument(
        "--fc_model",
        type=str,
        default="models/face_classifier_weights_best.pt",
        help="path to face classifier model if it was trained",
    )
    parser.add_argument(
        "--pre_split",
        type=str,
        help="path to pre_split file that encodes which video belongs to train and validation",
    )
    parser.add_argument(
        "--split_type",
        type=str,
        choices=["split0_train", "split0_test", "all"],
        default="split0_train",
    )
    parser.add_argument(
        "--one_video_per_child_policy",
        choices=[
            "include_all",
            "unique_only",
            "unique_only_in_val",
        ...
        type=str,
        default="unique_only_in_val",
        help="some videos are of the same child, this policy dictates what to do with those.",
    )
    parser.add_argument(
        "--train_val_disjoint",
        action="store_true",
        help="if true, train and validation sets will never contain same child",
    )
    parser.add_argument(
        "--val_percent",
        type=float,
        default=0.2,
        help="desired percent of validation set",
    )
    parser.add_argument(
        "--face_detector_confidence",
        type=float,
        default=0.7,
        help="confidence threshold for opencv_dnn face detector",
    )
    parser.add_argument(
        "--retinaface_confidence",
        type=float,
        default=0.9,
        help="confidence threshold for retinaface detector",
    )
    parser.add_argument(
        "--gpu_id", type=int, default=-1, help="Which GPU to use (or -1 for cpu)"
    )
    parser.add_argument("--log", help="if present, writes log to this path")
    parser.add_argument(
        "--seed", type=int, default=43, help="random seed (controls split selection)"
    )
    parser.add_argument(
        "-v",
        "--verbosity",
