...
def create_annotation_split(args, csv_name):
    ...
    for i in range(len(coding1_files)):
        shutil.copy(
            Path(coding1_files[i][1]),
            Path(coding1_location, coding1_files[i][0] + ".txt"),
        )
        shutil.copy(
            Path(coding2_files[i][1]),
            Path(coding2_location, coding2_files[i][0] + ".txt"),
        )
...
def build_marchman_video_dataset(raw_dataset_path, raw_dataset_type):
    if raw_dataset_type == "cali-bw":
        ...
        csv_location = Path(
            raw_dataset_path / "Senegal_Color_March_split0_participants.csv"
        )
    ...
    for entry in video_dataset.values():
        if entry["video_path"].name in csv_videos:
            ...
            entry["first_coding_file"] = (
                secondary_coder_folder / rows[index][codingfile1]
            )
            entry["second_coding_file"] = (
                primary_coder_folder / rows[index][codingfile2]
            )
    ...
    for entry in video_dataset.values():
        ...
        if entry[
            "has_2coding"
        ]:  # sanity: if there is a 2nd coding, a "1st" should exist otherwise the 2nd would be the 1st.
...
def build_lookit_video_dataset(raw_dataset_path, csv_location):
    ...
    # fill video dataset with information from folders
    first_coding_files = [
        f for f in Path(raw_dataset_path / "annotations" / "coder1").glob("*.txt")
    ]
    first_coding_files_video_ids = [
        "-".join(f.stem.split("_")[2].split("-")[1:]) for f in first_coding_files
    ]
    second_coding_files = [
        f for f in Path(raw_dataset_path / "annotations" / "coder2").glob("*.txt")
    ]
    second_coding_files_video_ids = [
        "-".join(f.stem.split("_")[2].split("-")[1:]) for f in second_coding_files
    ]
    for entry in video_dataset.values():
...
def preprocess_raw_lookit_dataset(args):
    ...
    valid_videos_in_csv = len(
        [x for x in video_dataset.values() if x["in_csv"] and x["has_1coding"]]
    )
    doubly_coded = len(
        [
            x
            for x in video_dataset.values()
            if x["in_csv"] and x["has_1coding"] and x["has_2coding"]
        ]
    )
    unique_children = len(
        np.unique(
    ...
    logging.info(
        ...
        " found videos in csv: {},"
        " doubly coded: {},"
        " unique children: {}".format(
            csv_videos, valid_videos, valid_videos_in_csv, doubly_coded, unique_children
        )
    )
    ...
    if args.split_type == "all":
        ...
        videos = [
            ...
            if x["in_csv"]
            and x["has_1coding"]
            and (x["split"] == "1_train" or x["split"] == "1_validate")
        ]
        ...
        if args.one_video_per_child_policy == "include_all":
            ...
            if args.train_val_disjoint:
                ...
                train_set = [
                    x
                    for x in videos
                    if x["child_id"] not in val_children_id and x not in val_set
                ]
            ...
            threshold = min(
                int(len(unique_videos) * args.val_percent), len(double_coded)
            )
            val_set = np.random.choice(double_coded, size=threshold, replace=False)
            ...
            double_coded_and_unique_videos = [
                x for x in videos[unique_indices] if x["has_2coding"]
            ]
            threshold = min(
                int(len(videos) * args.val_percent), len(double_coded_and_unique_videos)
            )
            val_set = np.random.choice(
                double_coded_and_unique_videos, size=threshold, replace=False
            )
            if args.train_val_disjoint:
                val_children_id = [x["child_id"] for x in val_set]
                train_set = [
            ...
            _, unique_indices = np.unique(
                temp_train_video_children_id, return_index=True
            )
    ...
    np.savez(
        split_path,
        [[x["video_id"] for x in train_set], [x["video_id"] for x in val_set]],
    )
    logging.info(
        "[preprocess_raw] training set: {} validation set: {}".format(
...
def preprocess_raw_marchman_dataset(args):
    ...
    if args.raw_dataset_type == "cali-bw":
        ...
        csv_file = Path(
            args.raw_dataset_path / "Senegal_Color_March_split0_participants.csv"
        )
    else:
        raise NotImplementedError
    # print some stats
    ...
    doubly_coded = len(
        [x for x in video_dataset.values() if x["in_csv"] and x["has_2coding"]]
    )
    unique_children = len(
        np.unique(
            [
    ...
    if args.split_type == "all":
        ...
        videos = [
            x
            for x in video_dataset.values()
            if x["in_csv"] and x["has_1coding"] and x["split"] == "2_test"
        ]
    else:
        raise NotImplementedError
    videos = np.array(videos)
...
def create_symbolic_links(train_set, val_set, args):
    for video_file in train_set:
        ...
        dst1 = Path(
            args.video_folder / (video_file["video_id"] + video_file_path.suffix)
        )
        os.symlink(str(src1), str(dst1))
        src2 = coding_file_path
        dst2 = Path(
            args.train_coding1_folder
            / (video_file["video_id"] + coding_file_path.suffix)
        )
        ...
        if video_file["second_coding_file"]:
            ...
            dst3 = Path(
                args.train_coding2_folder
                / (video_file["video_id"] + second_coding_file_path.suffix)
            )
    ...
    for video_file in val_set:
        ...
        src2 = coding_file_path
        dst2 = Path(
            args.val_coding1_folder / (video_file["video_id"] + coding_file_path.suffix)
        )
        os.symlink(str(src2), str(dst2))
        src3 = second_coding_file_path
        dst3 = Path(
            args.val_coding2_folder
            / (video_file["video_id"] + second_coding_file_path.suffix)
        )
        os.symlink(str(src3), str(dst3))
...
def process_dataset_lowest_face(args, gaze_labels_only=False, force_create=False):
    ...
    if args.fd_model == "retinaface":
        face_detector_model = RetinaFace(
            gpu_id=args.gpu_id, model_path=args.face_model_file, network=args.network
        )
    ...
    for video_file in video_list:
        ...
        gaze_labels_filename = Path.joinpath(
            args.faces_folder, video_file.stem, "gaze_labels.npy"
        )
        face_labels_filename = Path.joinpath(
            args.faces_folder, video_file.stem, "face_labels.npy"
        )
        if (
            gaze_labels_filename.exists()
            and face_labels_filename.exists()
        ...
        if vfr:
            logging.warning(
                "[process_lkt_legacy] video file: {} has variable frame rate".format(
                    str(video_file)
                )
            )
        ...
        if args.raw_dataset_type == "cali-bw" or args.raw_dataset_type == "senegal":
            ...
            parser = parsers.VCXParser(
                30, args.raw_dataset_path, args.raw_dataset_type, first_coder=True
            )
        elif args.raw_dataset_type == "lookit":
            csv_file = Path(
                args.raw_dataset_path / "prephys_split0_videos_detailed.tsv"
            )
            parser = parsers.LookitParser(
                fps, csv_file, first_coder=True, return_time_stamps=vfr
            )
        else:
            raise NotImplementedError
        ...
        while ret_val:
            if responses:
                logging.info(
                    "[process_lkt_legacy] Processing frame: {}".format(frame_counter)
                )
                if vfr:
                ...
                if start <= frame_stamp < end:  # only iterate on annotated frames
                    ...
                    if responses[response_index][1]:  # make sure response is valid
                        ...
                        if not gaze_labels_only:
                            ...
                            if args.fd_model == "retinaface":
                                ...
                                faces = [
                                    face
                                    for face in faces
                                    if face[-1] >= args.retinaface_confidence
                                ]
                                bbox = extract_bboxes(faces)
                            elif args.fd_model == "opencv_dnn":
                                bbox = detect_face_opencv_dnn(
                                    net, frame, args.face_detector_confidence
                                )
                            else:
                                raise NotImplementedError
                            if not bbox:
                                ...
                                logging.info(
                                    "[process_lkt_legacy] Video %s: Face not detected in frame %d"
                                ...
                                if args.use_facerec and fr.known_faces > 0:
                                    (
                                        ...
                                        feature_dict,
                                        resized_img,
                                    ) = fr.select_face_preprocessing(bbox, frame)
                                    ...
                                    img_filename = (
                                        img_folder / f"{frame_counter:05d}_{i:01d}.png"
                                    )
                                    if not img_filename.is_file() or force_create:
                                        cv2.imwrite(str(img_filename), resized_img)
                                    box_filename = (
                                        box_folder / f"{frame_counter:05d}_{i:01d}.npy"
                                    )
                                ...
                                if args.use_facerec and fr.known_faces == 0:
                                    new_bbox = fr.convert_bounding_boxes(
                                        [bbox[selected_face]]
                                    )
                        ...
                        face_labels.append(-2)
                        logging.info(
                            "[process_lkt_legacy] Skipping since frame is invalid"
                        )
                else:
                    no_annotation_counter += 1
                    gaze_labels.append(-2)
                    face_labels.append(-2)
                    logging.info(
                        "[process_lkt_legacy] Skipping since frame not in range of annotation"
                    )
            else:
                no_annotation_counter += 1
                gaze_labels.append(-2)
                face_labels.append(-2)
                logging.info(
                    "[process_lkt_legacy] Skipping frame since parser reported no annotation"
                )
            ret_val, frame = cap.read()
            frame_counter += 1
        if vfr:
...
def generate_second_gaze_labels(args, force_create=False, visualize_confusion=False):
    ...
    for video_file in video_list:
        ...
        gaze_labels_second_filename = Path.joinpath(
            args.faces_folder, video_file.stem, "gaze_labels_second.npy"
        )
        ...
        if vfr:
            logging.warning(
                "video file: {} has variable frame rate".format(str(video_file))
            )
            logging.info(str(meta_data))
            frame_info, vfr_frame_counter, _ = video.get_frame_information(video_file)

        if (
            not (args.train_coding1_folder / (video_file.stem + suffix)).exists()
            and not (args.train_coding2_folder / (video_file.stem + suffix)).exists()
            and not (args.val_coding1_folder / (video_file.stem + suffix)).exists()
            and not (args.val_coding2_folder / (video_file.stem + suffix)).exists()
        ):
            ...
            if args.raw_dataset_type == "cali-bw" or args.raw_dataset_type == "senegal":
                assert abs(fps - 30) < 0.1
                parser = parsers.VCXParser(
                    30, args.raw_dataset_path, args.raw_dataset_type, first_coder=False
                )
            elif args.raw_dataset_type == "lookit":
                csv_file = Path(
                    args.raw_dataset_path / "prephys_split0_videos_detailed.tsv"
                ...
                parser = parsers.LookitParser(
                    fps, csv_file, first_coder=False, return_time_stamps=vfr
                )
            else:
                raise NotImplementedError
            try:
            ...
            gaze_labels = np.load(
                str(
                    Path.joinpath(args.faces_folder, video_file.stem, "gaze_labels.npy")
                )
            )
            gaze_labels_second = []
...
def process_dataset_face_classifier(args, force_create=False):
    ...
    fc_args = Args()
    model, input_size = face_classifier.fc_model.init_face_classifier(
        fc_args, model_name=fc_args.model, num_classes=2, resume_from=args.fc_model
    )
    data_transforms = face_classifier.fc_eval.get_fc_data_transforms(
        fc_args, input_size
    )
    ...
    for video_file in tqdm(video_files):
        face_labels_fc_filename = Path.joinpath(
            args.faces_folder, video_file.stem, "face_labels_fc.npy"
        )
        if not face_labels_fc_filename.is_file() or force_create:
            ...
            face_labels = np.load(
                str(
                    Path.joinpath(args.faces_folder, video_file.stem, "face_labels.npy")
                )
            )
            face_labels_fc = []
            hor, ver = 0.5, 1
            for frame in tqdm(range(face_labels.shape[0])):
                if face_labels[frame] < 0:
                    ...
                    if centers.shape[0] == 0:
                        ...
                        dis = np.sqrt(
                            (centers[:, 0] - hor) ** 2 + (centers[:, 1] - ver) ** 2
                        )
                        i = np.argmin(dis)
...
def preprocess_raw_generic_dataset(args, force_create=False):
    ...
    coding_first = [
        "_".join(f.stem.split("_")[:-1]) for f in raw_coding_first_path.glob("*")
    ]
    coding_second = [
        "_".join(f.stem.split("_")[:-1]) for f in raw_coding_second_path.glob("*")
    ]
    ...
    test_set = (
        set(videos).intersection(set(coding_first)).intersection(set(coding_second))
    )
    for i, file in enumerate(sorted(list(training_set))):
        if not Path(args.video_folder, (file + ".mp4")).is_file() or force_create:
            shutil.copyfile(
                raw_videos_path / (file + ".mp4"), args.video_folder / (file + ".mp4")
            )
...
if __name__ == "__main__":
    ...
    gen_lookit_multi_face_subset(
        force_create=False
    )  # creates training set for face classifier
