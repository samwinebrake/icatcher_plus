...
def get_args(args=None):
    ...
    )  # can only be applied to resnet & densenet
    # comments
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="regularize loss based on estimated age",
    )
    # resume from a path
    ...
    parser.add_argument(
        "--dataset_folder",
        default="",
        help="path to processed dataset folder (not the raw one)",
    )
...
def train_face_classifier(
    ...
    beta_dist = (
        1 - (beta_dist - beta_dist.min()) / np.max(beta_dist - beta_dist.min())
    ) / 10
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels, ages in tqdm(dataloaders[phase], desc=phase):
                ...
                loss_weights = (
                    1
                    + torch.from_numpy(np.asarray(beta_dist[ages]))
                    .to(args.device)
                    .float()
                )
    ...
    if plot_lr_curve:
        visualize.plot_learning_curve(
            train_loss_history, val_loss_history, save_dir=save_dir, isLoss=True
        )
        visualize.plot_learning_curve(
            train_acc_history, val_acc_history, save_dir=save_dir, isLoss=False
        )
...
if __name__ == "__main__":
    ...
    model, input_size = init_face_classifier(
        args, model_name=model_name, num_classes=num_classes, resume_from=resume_from
    )
    dataloaders = get_dataset_dataloaders(
        args, input_size, batch_size, shuffle_datasets
    )
    criterion = get_loss()
