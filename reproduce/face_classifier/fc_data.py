...
def create_train_test(raw_data_folder, train_val_split=0.7):
    ...
    train = np.random.choice(
        indices, size=int(my_range * train_val_split), replace=False
    )
    ...
    for index in train:
        shutil.copyfile(
            Path(infant_folder, infants[index]),
            Path(dirs["train"]["infant"], infants[index]),
        )
        shutil.copyfile(
            Path(non_infant_folder, non_infants[index]),
            Path(dirs["train"]["non_infant"], non_infants[index]),
        )
    for index in val:
        shutil.copyfile(
            Path(infant_folder, infants[index]),
            Path(dirs["val"]["infant"], infants[index]),
        )
        shutil.copyfile(
            Path(non_infant_folder, non_infants[index]),
            Path(dirs["val"]["non_infant"], non_infants[index]),
        )
    for index in test:
        shutil.copyfile(
            Path(infant_folder, infants[index]),
            Path(dirs["test"]["infant"], infants[index]),
        )
        shutil.copyfile(
            Path(non_infant_folder, non_infants[index]),
            Path(dirs["test"]["non_infant"], non_infants[index]),
        )
...
def get_dataset_dataloaders(args, input_size, batch_size, shuffle=True, num_workers=0):
    ...
    image_datasets = {
        "train": ImageFolderWithAge(
            str(Path(face_data_folder, "train")), data_transforms["train"]
        ),
        "val": ImageFolderWithAge(
            str(Path(face_data_folder, "val")), data_transforms["val"]
        ),
        "test": ImageFolderWithAge(
            str(Path(face_data_folder, "test")), data_transforms["test"]
        ),
    }
