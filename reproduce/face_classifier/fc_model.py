    ...
    if model_name == "vgg11":
        ...
        if args.dropout > 0:
            model.fc = nn.Sequential(
                nn.Dropout(args.dropout), nn.Linear(num_ftrs, num_classes)
            )
...
def make_optimizer_and_scheduler(args, model):
    ...
    if arg_optim == "adam":
        ...
        optimizer = optim.SGD(
            params_to_update, lr=args.lr, momentum=0.9, weight_decay=1e-4
        )
    else:
    ...
    if args.scheduler == "exp":
        ...
        scheduler = lr_scheduler.MultiStepLR(
            optimizer=optimizer, milestones=[10, 20], gamma=0.1
        )
