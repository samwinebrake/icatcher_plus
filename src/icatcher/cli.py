...
def detect_face_opencv_dnn(net, frame, conf_threshold):
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > conf_threshold:
            ...
            if (
                x1 >= frameWidth or y1 >= frameHeight
            ):  # if they are larger than image size, bbox is invalid
                continue
            x2 = min(
                int(detections[0, 0, i, 5] * frameWidth), frameWidth
            )  # either right side of box or frame width
            y2 = min(
                int(detections[0, 0, i, 6] * frameHeight), frameHeight
...
def load_models(opt):
    ...
    GOODBOY = pooch.create(
        ...
        registry={
            "zip_content.txt": "d81bfb5a183edea6dc74f7f342d516a9843865570b9ecfbf481209ec5114110a",
